const vinted = require("./lib/index.js");
const fs = require("fs");
const EventEmitter = require("eventemitter3");
const { parseVintedURL } = require("./lib/index.js");
...
class VintedViewver extends EventEmitter {
  ...
  #proxy(listOrFile) {
    ...
    if (typeof listOrFile === "object" && Array.isArray(listOrFile))
      ...
      return fs.readFileSync(listOrFile, "utf8")?.split("\n");
      console.log(fs.readFileSync(listOrFile)?.split("\n"));
    } else return;
  }
  #search() {
    return new Promise(async (resolve) => {
      try {
        ...
        if (this.debug)
          console.log(
            `${itemsToNotify.length} nouveaux items a notifier sur ${url}!\n`
          );
